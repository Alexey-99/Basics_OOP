...
public class ProgramProcess {
	 ...
	 * salesman.printProducts(); System.out.println(salesman.getProducts().length +
	 * OPERATION_EXIT); }
	 */

	/*
	 * @SuppressWarnings("resource") private static int
	 * enterNumberProductForBuy(Person salesman, Person customer) { int number = 0;
	 * StringBuilder builder = new StringBuilder();
	 * builder.append(customer.lastFirstNamePatronymic()).append(SPACE);
	 * builder.append(MASSEGE_ENTER_PRODUCT_NUMBER); System.out.println(builder); do
	 * { Scanner scan = new Scanner(System.in); if (scan.hasNextInt()) { number =
	 * scan.nextInt(); } else { System.out.println(MESSAGE_ERROW); } } while
	 * (Validation.validationEnteredNumberForBuy(number, salesman) == false); return
	 * number; }
	 */
	...
	private static void selectionProductsForBuy(Person salesman, Person customer) {
		...
		/*
		 * int numberProduct = 0; do { printProductsSalesman(salesman); numberProduct =
		 * enterNumberProductForBuy(salesman, customer); if (numberProduct <
		 * salesman.getProducts().length) {
		 * customer.addProduct(salesman.getProducts()[numberProduct]);
		 * salesman.getProducts()[numberProduct].setIsStatus(STATUS_RESERVED); } } while
		 * (numberProduct < salesman.getProducts().length);
		 */
	}
}